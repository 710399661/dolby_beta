...
public class ScriptHelper {
    ...
    public static void startScript() {
        ...
        String startCommand = "cd " + scriptPath + " && " +
                nodeLibPath + " && " + nodeBinPath + " && " +
                "export ENABLE_FLAC=\"" + settingHelper.getSetting(SettingHelper.proxy_flac_key) + "\" && " +
                "export MIN_BR=\"" + (settingHelper.getSetting(SettingHelper.proxy_priority_key) ? "256000" : "96000") + "\" && " +
                "export QQ_COOKIE=\"" + settingHelper.getQqCookie() + "\" && " +
                "export MIGU_COOKIE=\"" + settingHelper.getMiguCookie() + "\" && " +
                "libnode.so app.js -a 127.0.0.1 -o " + settingHelper.getProxyOriginal() +
                " -p " + settingHelper.getProxyPort() + ":" + (settingHelper.getProxyPort() + 1);
        Command start = new Command(0, STOP_PROXY[0], STOP_PROXY[1], startCommand) {
            @Override
